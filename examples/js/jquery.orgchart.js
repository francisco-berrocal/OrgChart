...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    switch (options) {
      ...
      case 'addSatellite':
        return addSatellite.apply(this, Array.prototype.splice.call(arguments, 1));
    ...
    if (opts.pan) {
      ...
      $chart.on('mousedown',function(e){
        var $this = $(this);
        if ($(e.target).closest('.node').length || $(e.target).closest('.satellite').length) {
  ...
  function createNode(nodeData, level, opts) {
    ...
    if (opts.draggable) {
      ...
      .on('dragover', function(event) {
        ...
        var $satellite = $dropZone.closest('.orgchart').data('satellite');
        if ($satellite != null) {
          if(opts.dropSatelliteCriteria &&
            !opts.dropSatelliteCriteria($satellite, $dropZone)) 
      ...
      .on('drop', function(event) {
        ...
        var $satellite = $dropZone.closest('.orgchart').data('satellite');
        if($satellite != null) {
          $dropZone.parent().append($satellite)
          $orgchart.triggerHandler({ 'type': 'satelittedropped.orgchart', 'dropZone': $dropZone, 'satellite': $satellite});
          return
        }
  ...
  function buildHierarchy ($appendTo, nodeData, level, opts, callback) {
    ...
    if (Object.keys(nodeData).length > 1) { // if nodeData has nested structure
      ...
      .done(function($nodeDiv) {
        var $tr = $nodeDiv.wrap('<tr><td' + (hasChildren ? ' colspan="' + $childNodes.length * 2 + '"' : '') + '></td></tr>').closest('tr');
        if (opts.createSatellite) {
          var satellites = opts.createSatellite($nodeDiv, nodeData)
          addSatellite($tr, satellites, opts)
        }
        $table.append($tr); 
        if (callback) {
          callback();
  ...
  function addSatellite($el, satellites, opts) {
    ...
    satellites.forEach(function(satellite) {
      ...
      .on('dragstart', function (e) { 
          var $dragged = $(this);
          $dragged.closest('.orgchart')
            .data('satellite', $dragged)
            .find('.node').each(function(index, node) {
              if(opts.dropSatelliteCriteria) {
                if (opts.dropSatelliteCriteria($dragged, $(node)))
                  $(node).addClass('allowedDrop');
              } else
                  $(node).addClass('allowedDrop');
            });
      ...
      .on('dragend', function (e) { 
        $(this).closest('.orgchart')
          .data('satellite', null)
          .find('.allowedDrop').removeClass('allowedDrop');
      })
    })
    $container.append(satellites)
  }
